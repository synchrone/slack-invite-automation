...
module.exports = {
    badge: function(presence, total, customColorA, customColorB) {
      ...
      <g fill="#fff" text-anchor="middle" font-family="DejaVu Sans,Verdana,Geneva,sans-serif" font-size="110">
        <text x="${pl}" y="150" fill="#010101" fill-opacity=".3" transform="scale(.1)" textLength="${title.length*lw}">${title}</text>
        <text x="${pl}" y="140" transform="scale(.1)" textLength="${title.length*lw}">slack</text>
        <text x="${pl-20 + wd*5}" y="150" fill="#010101" fill-opacity=".3" transform="scale(.1)" textLength="${title.length*lw}">${value}</text>
        <text x="${pl-20 + wd*5}" y="140" transform="scale(.1)" textLength="${title.length*lw}">${value}</text>
      </g>
    </svg>`
