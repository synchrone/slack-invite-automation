...
async function sendVerification(phone, ratelimitKey) {
    ...
    try {
        ...
        console.log({carrier: carrierInfo.carrier, cc: carrierInfo.countryCode})
        if (carrierInfo.carrier.type !== 'mobile' || config.allowedCountries.indexOf(carrierInfo.countryCode) === -1) {
